...
import { FileType } from '../../../shared/getPageFiles/fileTypes.js'
import { getFilePathAbsolute, removeFileExtention, unique } from '../utils.js'
...
async function getEntries(config: ResolvedConfig): Promise<Record<string, string>> {
  ...
  const pageFileEntries = await getPageFileEntries(config, false) // TODO/v1-release: remove
  console.log('pageFileEntries', pageFileEntries)
...
async function getPageFileEntries(config: ResolvedConfig, includeAssetsImportedByServer: boolean) {
  ...
  let pageFiles = await findPageFiles(config, "docs")
